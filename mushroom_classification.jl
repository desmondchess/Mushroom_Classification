...
begin
	...
	function viz_confusion(cm::Matrix; class="edible")
		cm_to_plot = reverse(cm, dims=1)'
	
		fig1 = Figure()
		ax1  = Axis(fig1[1, 1], 
			xlabel="prediction", ylabel="truth",
			xticks=(1:2, ["not $class", "$class"]),
			yticks=(1:2, reverse(["not $class", "$class"]))
		)
	    ...
	    Colorbar(fig1[1, 2], hm, label="# mushrooms")
		return fig1
	end
end
...
mushroom = CSV.read("agaricus-lepiota.data", DataFrame, header = header)
...
attribute = ncol(mushroom)-1 # -1 indicating the removal of the first column (class)
...
# ╔═╡ e62b8e15-096b-48a0-bc84-8bca62859502
begin
	mushroom_missing = CSV.read("agaricus-lepiota.data", DataFrame, header = header, missingstring = "?")
	dropmissing(mushroom_missing)
end

# ╔═╡ 48cb6573-d126-4571-8846-fc15f0ea801b
md"🍄 use `combine` and `groupby` to determine how many of the mushrooms are edible vs. not."

# ╔═╡ 04fa7c7f-6632-4bc2-b579-ad83ecd0be86
mushroom_df = mushroom

# ╔═╡ b5c65b95-716f-41ca-8020-f76d008f15a7
transform!(mushroom_df, "class" => (col -> replace(col, "p" => "poisonous", "e" => "edible")), renamecols = false)


# ╔═╡ 0e17ce4a-52f8-471a-abac-b4f4e70e334d
transform!(mushroom_df, "odor" => (col -> replace(col, "a"=>"almond","l"=>"anise","c"=>"creosote","y"=>"fishy","f"=>"foul","m" =>"musty","n"=>"none","p"=>"pungent","s"=>"spicy")), renamecols = false)

# ╔═╡ 98a2d321-06f6-46b8-9f05-339eeaf00401
sorted_mushrooms = combine(groupby(mushroom_df, "class"), "class" => (col -> length(col)) => "# of Mushrooms")
...
begin
	unique_categories = [unique(mushroom[:,i]) for i = 2:23]
	sum_uni_cat = sum([length(x) for x in unique_categories])
...
begin
	...
	axis = (width = 550, height = 350, title= "Mushroom Data Odor Distribution", ylabel="# of Mushrooms")
	mushroom_frequency = data(mushroom_df) * frequency() * mapping(:odor)
	
	draw(mushroom_frequency; axis)
end

# ╔═╡ a6abd64b-b826-430f-ae90-58a51dfd4e5d
begin
	plt = mushroom_frequency * mapping(color = :class, dodge = :class)
	draw(plt; axis)
...
# ╔═╡ e8328034-95c8-4617-ab78-396a58ff9558
unique(mushroom_df[:,header[2]])

# ╔═╡ e8a3b53c-031c-4414-b2ab-32beb0be689d
begin
	for unique_index in 2:length(header)
		for attr in unique(mushroom_df[:,header[unique_index]])
			transform!(mushroom_df, header[unique_index] .=> (col -> col .== attr) => (header[unique_index] * "_" * attr))
		end
	end
	mushroom_df
end

# ╔═╡ e8aaf663-e9ce-4a0f-aed4-caae403e8acd
size(mushroom_df)

# ╔═╡ e2e091c2-5001-4d0f-9ab8-79d26ef12e1c
new_feature = select(mushroom_df, r"_")

# ╔═╡ 3e4f5166-00c4-4a64-b4c6-3f266446ec72
size(new_feature)
...
# ╔═╡ 8c314af0-7bff-4480-824b-4313a44e57ce
 feature_matrix_X = Int.(new_feature)

# ╔═╡ 295ecef8-814d-42d2-8fb2-2e4d1fb9eb44
new_feature_X = Matrix(feature_matrix_X)
...
target_vector_y = mushroom_df[:, header[1]]
...
# ╔═╡ 00c27f47-caea-41bc-8bca-6df053303ed0
model.fit(new_feature_X,target_vector_y)
...
# ╔═╡ 52883646-8fd0-4e82-b34a-57242c6a22db
model.estimators_[99]

# ╔═╡ 7c48ef30-2779-41ee-9125-adb6879e313d
each_depth = [depth.get_depth() for depth in model.estimators_]

# ╔═╡ 4cdd257a-68d4-4a44-aabe-258cccb202a1
begin
	fig = Figure()
	ax = Axis(fig[1, 1], 
	title="Depth of Model Trees", 
	xlabel="Each Depth",
	ylabel="# Depth"	)
	hist!(each_depth, color="limegreen")
	ylims!(0, nothing)
	fig
end
...
1. each tree is trained on...
	trained on different sets of data with replacements(which is as a result of bagging or bootstrap aggregation)
2. at each split within a tree, ...
	different features are used to make decisions(as a result of feature randomness).
	we could say each split within a tree undergoes a train test split function.
"
...
oob_predict = model.oob_decision_function_

# ╔═╡ 7eafd50e-5b8d-42a3-bae5-ea89b53a5e82
oob_p = Int.(oob_predict[:,2] .>= 0.5)

# ╔═╡ 8a8bc469-037e-43ce-87db-3bf24a2f5f3d
begin
	target_y = target_vector_y .== "edible" 
	y_target = Int.(target_y)
end

# ╔═╡ 8c42e3b4-e36a-44b8-b701-3d9e18e70ea1
conf_matrix1 = confusion_matrix(y_target,oob_p)

# ╔═╡ 42b467d0-994a-4b5b-8b43-adb5fa0d4c40
viz_confusion(conf_matrix1)
...
predict which feature or features of a given mushroom was not considered at a decision tree after bagging occurs. furthermore, A prediction made for a feature in the original data set using only decision trees not trained on this particular feature is called out-of-bag (OOB) prediction. These predictions are not prone to overfitting, as each prediction is only made by trees that did not use the feature for training.
"
...
feat_name = names(feature_matrix_X)

# ╔═╡ c56f6b78-3007-4c7f-b2ff-4c559d3fae1c
import_feat_df = DataFrame("feat_name" => feat_name, "feat_score" => feat_score)

# ╔═╡ 6f573395-c38c-47f9-a18e-38133fa311ba
top_most_imp_feat = import_feat_df[partialsortperm(import_feat_df.feat_score, 1:10, rev=true), :]

# ╔═╡ 8e0d6238-bae1-48f9-ad39-1ca2d6914044
begin
	local fig = Figure()
	xs = top_most_imp_feat[:,2]
	ys = 1:length(top_most_imp_feat[:,2])
	local ax = Axis(fig[1,1], title = "Ten Most Important Features", ylabel = "The ten most important features", xlabel = "the importance score of those features")
	barplot!(ax, ys, xs, color=xs, direction=:x, bar_labels=top_most_imp_feat[:,1])
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "cf14f53491f4dbf472678f50c93c4d38c1cdeace"
